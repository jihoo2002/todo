...
@EnableGlobalMethodSecurity(prePostEnabled = true) //메서드 실행 전에 권한 검사해주는 아노테이션
public class WebSecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http
                ...
                .csrf().disable()
                .httpBasic().disable()
                // 세션인증을 사용하지 않겠다., 세션을 사용하지 않기 때문에 STATELESS로 설정
                .sessionManagement()
                .sessionCreationPolicy(SessionCreationPolicy.STATELESS)
                .and()
                .authorizeRequests() // 어떤 요청에서 인증을 안 할 것인지, 언제 인증을 할 것인지 설정
