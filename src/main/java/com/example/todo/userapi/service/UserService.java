...
//        }
    ...
    public LoginResponseDTO kakaoService(final String code) {
        ...
        if(!isDuplicate(dto.getKakaoAccount().getEmail())) {
            //이메일이 중복되지 않았디 -> 이전에 로그인 한 적이 앖음 -> DB에 데이터 세팅
            //->이메일이 중복되지 않았다면
            User saved = userRepository.save(dto.toEntity((String) responseData.get("access_token")));

         }
           //이메일이 중복됐다? ->이전에 로그인 한 적이 있따 -> DB에 데이터를 또 넣을 필요는 없다
        ...
        foundUser.setAccessToken((String) responseData.get("access_token")); //사용자의 토큰을 갈아끼움, 만약 데베에 이전 토큰이 저장되어 있다면!
    ...
    public String logout(TokenUserInfo userInfo) {
        User fountUser = userRepository.findById(userInfo.getUserId()).orElseThrow();
        String accessToken = fountUser.getAccessToken();  //컨트롤 + alt + v = 변수화 한다.
        if(accessToken != null) {
            String reqUri = "https://kapi.kakao.com/v1/user/logout";
            HttpHeaders headers = new HttpHeaders();
            headers.add("Authorization", "Bearer " + accessToken); //토큰 만료시키기
